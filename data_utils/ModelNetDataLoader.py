...
class ModelNetDataLoader(Dataset):
    def __init__(self, root, args, split='train', process_data=False):
        ...
        shape_ids['train'] = [[(line.strip().split(','))[1],(line.strip().split(','))[3]] for line in open(os.path.join(self.root, 'metadata_modelnet40.csv')) if 'train' in line]
        shape_ids['test'] = [[(line.strip().split(','))[1],(line.strip().split(','))[3]] for line in open(os.path.join(self.root, 'metadata_modelnet40.csv')) if 'test' in line]

        self.classes = sorted(list(set([(line.strip().split(','))[1] for line in open(os.path.join(self.root, 'metadata_modelnet40.csv')) if 'test' in line])))

...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_scores = attention_scores / (512**0.5)
        attention_weights = torch.nn.functional.softmax(attention_scores, dim=-1)  # [batch_size, seq_len, seq_len]

        out = torch.bmm(attention_weights, V)  # [batch_size, seq_len, feature_dim]
...
class get_model(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # self attention l1_points
        self_attention = SelfAttention(1024).to('cuda')

        # Forward Pass
        output, attention_weights = self_attention(l1_points.permute(0, 2, 1))
        l1_points = output.permute(0,2,1)
